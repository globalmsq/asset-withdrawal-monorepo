...
const chain_provider_factory_1 = require("../../providers/chain-provider.factory");
// Mock ethers
jest.mock('ethers', () => ({
    ethers: {
...
describe('ChainProvider', () => {
    ...
    describe('constructor', () => {
        it('should create a provider for Polygon mainnet', () => {
            const provider = new chain_provider_1.ChainProvider({
                chain: 'polygon',
                network: 'mainnet',
            });
        ...
        it('should use custom RPC URL if provided', () => {
            ...
            expect(ethers_1.ethers.JsonRpcProvider).toHaveBeenCalledWith(customRpcUrl, 137);
        });
        it('should throw error for unsupported chain', () => {
            expect(() => {
                new chain_provider_1.ChainProvider({
                    chain: 'unsupported',
                    network: 'mainnet',
                });
    ...
    describe('methods', () => {
        let provider;
        beforeEach(() => {
            provider = new chain_provider_1.ChainProvider({
                chain: 'polygon',
                network: 'mainnet',
            });
        });
...
describe('ChainProviderFactory', () => {
    beforeEach(() => {
        chain_provider_factory_1.ChainProviderFactory.clearProviders();
    });
    it('should create and cache Polygon provider', () => {
        const provider1 = chain_provider_factory_1.ChainProviderFactory.getProvider('polygon', 'mainnet');
        const provider2 = chain_provider_factory_1.ChainProviderFactory.getProvider('polygon', 'mainnet');
    ...
    it('should create and cache Ethereum provider', () => {
        const provider1 = chain_provider_factory_1.ChainProviderFactory.getProvider('ethereum', 'testnet');
        const provider2 = chain_provider_factory_1.ChainProviderFactory.getProvider('ethereum', 'testnet');
        expect(provider1).toBe(provider2); // Should return cached instance
        expect(provider1.chain).toBe('ethereum');
        expect(provider1.network).toBe('testnet');
    });
    it('should create and cache BSC provider', () => {
        const provider1 = chain_provider_factory_1.ChainProviderFactory.getProvider('bsc', 'mainnet');
        const provider2 = chain_provider_factory_1.ChainProviderFactory.getProvider('bsc', 'mainnet');
        expect(provider1).toBe(provider2); // Should return cached instance
        expect(provider1.chain).toBe('bsc');
        expect(provider1.network).toBe('mainnet');
    });
    it('should create different instances for different networks', () => {
        const mainnetProvider = chain_provider_factory_1.ChainProviderFactory.getProvider('polygon', 'mainnet');
        const testnetProvider = chain_provider_factory_1.ChainProviderFactory.getProvider('polygon', 'testnet');
    ...
    it('should create different instances for custom RPC URLs', () => {
        const defaultProvider = chain_provider_factory_1.ChainProviderFactory.getProvider('polygon', 'mainnet');
        const customProvider = chain_provider_factory_1.ChainProviderFactory.getProvider('polygon', 'mainnet', 'https://custom-rpc.com');
...
//# sourceMappingURL=chain.provider.test.js.map