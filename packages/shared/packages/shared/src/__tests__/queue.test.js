...
describe('InMemoryQueue', () => {
    let queue;
    beforeEach(() => {
        queue = new queue_1.InMemoryQueue('test-queue');
    });
    describe('enqueue', () => {
        ...
        it('should handle multiple data types', async () => {
            const objectQueue = new queue_1.InMemoryQueue('object-queue');
...
describe('QueueManager', () => {
    let manager;
    beforeEach(() => {
        manager = new queue_1.QueueManager();
    });
    describe('getQueue', () => {
        it('should create new queue if not exists', () => {
            const queue = manager.getQueue('test-queue');
            expect(queue).toBeInstanceOf(queue_1.InMemoryQueue);
            expect(queue.getQueueSize()).toBe(0);
        });
        ...
        it('should support generic types', async () => {
            const stringQueue = manager.getQueue('string-queue');
            const numberQueue = manager.getQueue('number-queue');
...
describe('queueManager singleton', () => {
    it('should be instance of QueueManager', () => {
        expect(queue_1.queueManager).toBeInstanceOf(queue_1.QueueManager);
    });
    it('should maintain state across calls', async () => {
        const queue = queue_1.queueManager.getQueue('singleton-test');
        await queue.enqueue('test data');
        const sameQueue = queue_1.queueManager.getQueue('singleton-test');
