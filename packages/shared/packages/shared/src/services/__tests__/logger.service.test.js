...
describe('LoggerService', () => {
    ...
    it('should exist', () => {
        expect(logger_service_1.LoggerService).toBeDefined();
    });
    describe('Initialization', () => {
        it('should initialize with default configuration', () => {
            const defaultLogger = new logger_service_1.LoggerService();
            expect(defaultLogger).toBeDefined();
            expect(defaultLogger.getWinstonLogger()).toBeDefined();
        });
        it('should initialize with custom configuration', () => {
            const customLogger = new logger_service_1.LoggerService({
    ...
    describe('Logging levels', () => {
        it('should log info messages', () => {
            ...
            expect(mockWinstonLogger.info).toHaveBeenCalledWith('Test info message', expect.objectContaining({ service: 'test-service' }));
        });
        it('should log error messages', () => {
            logger.error('Test error message');
            expect(mockWinstonLogger.error).toHaveBeenCalledWith('Test error message', expect.objectContaining({ service: 'test-service' }));
        });
        it('should log error messages with Error object', () => {
            const error = new Error('Test error');
            logger.error('Test error with object', error);
            expect(mockWinstonLogger.error).toHaveBeenCalledWith('Test error with object', expect.objectContaining({
                service: 'test-service',
                error: error,
            }));
        });
        it('should log warn messages', () => {
            ...
            expect(mockWinstonLogger.warn).toHaveBeenCalledWith('Test warning message', expect.objectContaining({ service: 'test-service' }));
        });
        it('should log debug messages', () => {
            logger.debug('Test debug message');
            expect(mockWinstonLogger.debug).toHaveBeenCalledWith('Test debug message', expect.objectContaining({ service: 'test-service' }));
        });
    });
