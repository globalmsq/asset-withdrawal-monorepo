...
export class NonceManager {
  ...
  async initialize(address: string): Promise<void> {
    ...
    this.logger.info('Nonce manager initialized', {
      address,
  ...
  async getNextNonce(): Promise<number> {
    ...
    const lockPromise = (async () => {
      try {
        ...
        this.logger.debug('Allocated nonce', {
          nonce,
  ...
  markNoncePending(nonce: number): void {
    ...
    this.logger.debug('Marked nonce as pending', {
      nonce,
      total: this.pendingNonces.size,
    });
  }

  markNonceConfirmed(nonce: number): void {
    this.pendingNonces.delete(nonce);
    this.logger.debug('Marked nonce as confirmed', {
      nonce,
      remaining: this.pendingNonces.size,
    });
  }
