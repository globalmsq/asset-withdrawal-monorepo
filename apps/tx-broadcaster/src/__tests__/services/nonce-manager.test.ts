...
describe('NonceManager', () => {
  ...
  describe('Nonce 순서 정렬 처리', () => {
    it('순서가 바뀐 트랜잭션을 자동으로 정렬해야 함', async () => {
      ...
      // 큐 상태 확인
      const status = nonceManager.getQueueStatus();
      expect(status).toHaveLength(1);
      expect(status[0].pendingCount).toBe(3);

      // 내부 큐 확인 (정렬되어 있어야 함)
      const queue = (nonceManager as any).pendingTransactions.get(address);
    ...
    it('큰 Nonce가 먼저 도착해도 작은 Nonce를 기다려야 함', async () => {
      ...
      // Gap 정보 확인
      const gapInfo = nonceManager.getNonceGapInfo(address);
  ...
  describe('Nonce Gap 정보 제공', () => {
    it('Nonce Gap이 없을 때 올바른 정보를 반환해야 함', () => {
      const address = '0x1234567890123456789012345678901234567890';

      // 트랜잭션이 없을 때
      let gapInfo = nonceManager.getNonceGapInfo(address);
      expect(gapInfo).toBeNull();

      // 트랜잭션 추가
      nonceManager.addTransaction({
        txHash: 'hash1',
        nonce: 1,
        signedTx: 'tx1',
        requestId: 'req1',
      ...
      // 첫 트랜잭션이므로 Gap 없음
      gapInfo = nonceManager.getNonceGapInfo(address);
  ...
  describe('타임아웃 처리', () => {
    it('타임아웃된 트랜잭션을 해제해야 함', async () => {
      ...
      // 수동으로 타임아웃 시간 설정 (테스트용)
      const processingStartTime = (nonceManager as any).processingStartTime;
      processingStartTime.set(address, new Date(Date.now() - 70000)); // 70초 전

      // 타임아웃 체크 및 해제
      await (nonceManager as any).checkAndReleaseTimedOutTransactions();
