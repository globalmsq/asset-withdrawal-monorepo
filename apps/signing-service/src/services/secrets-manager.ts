...
export class SecureSecretsManager {
  ...
  private async loadPrivateKey(): Promise<void> {
    try {
      ...
      this.logger.auditFailure('LOAD_PRIVATE_KEY', error instanceof Error ? error.message : String(error), {});
      throw error;
    }
  }
  ...
  private async getSecret(secretName: string): Promise<any> {
    ...
    try {
      ...
      if (this.config.nodeEnv === 'development' && secretName.includes('private-key')) {
